...
void getTotalTrials(int *cnt, int rank) {
  if (rank == 0) {
    ...
    scanf("%d", cnt);
    // Broadcast to the other threads
  }
 ...
 * 				cardCount: The number of valid cards in the hand
 * so far Returns: non-zero if card is in the hand, zero otherwise
 */
...
/*makeStraightFlush1
 * Description: Debugging function to generate a straight flush starting at 1
 * pre-sorted Arguments: hand: An array of cards to store the result in Returns:
 * None
 */
 ...
 * unsorted Arguments: hand: An array of cards to store the result in Returns:
 * None
 */
...
int main(int argc, char **argv) {
  ...
  srand(time(0) + comm_rank);

  // get the number of trials per process
  int local_count, total_count;
  getTotalTrials(&total_count, comm_rank);
  local_count = total_count / comm_size;
  ...
  int total_flushes;
  MPI_Reduce(&straightFlushes, &total_flushes, 1, MPI_INT, MPI_SUM, 0,
             MPI_COMM_WORLD);

  double end_time = MPI_Wtime();

  if (comm_rank == 0) {
    percent = (float)total_flushes / (float)total_count * 100.0;
